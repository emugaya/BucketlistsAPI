...
from app.exceptions import NotFoundException, ValueErrorException
...
class BucketListItems(Resource):
    ...
    def put(self, bucketlist_id, item_id):
        ...
        if not name:
            raise ValueErrorException("Please Supply Name and done status.")
        ...
        if not bucketlist:
            raise NotFoundException("Bucketlist ID " + bucketlist_id + " is incorrect")

        bucketlist_item = Item.query.filter(Item.id == item_id).first()
        if not bucketlist_item:
            raise NotFoundException("Item " + item_id + "Doesn't Exist")
