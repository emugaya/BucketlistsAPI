...
from flask import abort, request, jsonify, make_response,g
...
class CreateItem(Resource):
    ...
    def post(self, bucketlist_id):
        ...
        name = args.name
        if not name:
            raise BadRequest("Please provide a name for your item")
        ...
        if bucketlist_item:
            raise Conflict("Item with this name already exits")
...
class UpdateAndDeleteItem(Resource):
    ...
    def patch(self, bucketlist_id, item_id):
        ...
        if not name:
            raise BadRequest("Please Supply Name and done status.")
